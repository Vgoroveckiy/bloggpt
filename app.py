...
if not openai.api_key or not currentsapi_key:
    raise ValueError(
        "Переменные окружения OPENAI_API_KEY и CURRENTS_API_KEY должны быть установлены"
    )
...
def get_recent_news(topic: str):
    ...
    if response.status_code != 200:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при получении данных: {response.text}"
        )
...
def generate_content(topic: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при генерации контента: {str(e)}"
        )
